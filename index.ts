...
import { complete, getModel, type Model, StringEnum } from "@earendil-works/pi-ai";
...
import { buildDeterministicSummary, generateSummaryDraft, type SummaryGenerationContext, type SummaryMeta } from "./summary-review.js";
...
async function openInBrowser(pi: ExtensionAPI, url: string): Promise<void> {
	const plat = platform();
	const result =
		plat === "darwin" ? await pi.exec("open", [url]) : plat === "win32" ? await pi.exec("cmd", ["/c", "start", "", url]) : await pi.exec("xdg-open", [url]);
	if (result.code !== 0) {
...
function openInGlimpse(open: (html: string, opts: Record<string, unknown>) => GlimpseWindow, url: string, title: string): GlimpseWindow {
...
function formatEntryLine(entry: ActivityEntry, theme: { fg: (color: string, text: string) => string }): string {
	...
	const target =
		entry.type === "api" ? `"${truncateToWidth(entry.query || "", 28, "")}"` : truncateToWidth(entry.url?.replace(/^https?:\/\//, "") || "", 30, "");
			...
			handle = await startCuratorServer(
				...
				},
				{
					...
					async onAddSearch(query, queryIndex, provider) {
						...
						const requestedProvider = !normalizedProvider || normalizedProvider === "auto" ? pc.defaultProvider : normalizedProvider;
						try {
							const {
								answer,
								results,
		...
		parameters: Type.Object({
			...
			workflow: Type.Optional(
				StringEnum(["none", "summary-review"], {
					description: "Search workflow mode: none = return results directly (default), summary-review = open curator with AI summary draft",
				}),
			),
		}),
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			for (let i = 0; i < queryList.length; i++) {
				...
				try {
					...
					const message = err instanceof Error ? err.message : String(err);
					const requestedProvider = typeof resolvedProvider === "string" && resolvedProvider !== "auto" ? resolvedProvider : undefined;
					searchResults.push({
						query,
						answer: "",
						results: [],
				...
				const handle = await startCuratorServer(
					...
					},
					{
						...
						async onAddSearch(query, queryIndex, provider) {
							...
							const normalizedProvider = normalizeProviderInput(provider);
							const requestedProvider = !normalizedProvider || normalizedProvider === "auto" ? currentProvider : normalizedProvider;
							try {
								const {
									answer,
									results,
