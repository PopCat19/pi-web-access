...
function resolveProvider(
	...
	const provider =
		normalizeProviderInput(requested ?? loadConfig().provider ?? "auto") ??
		"auto";
...
function formatQueryHeader(
	...
	const suffix =
		duplicateQueries.has(query) && provider ? ` (${provider})` : "";
...
interface GlimpseWindow {
	...
	on(
		event: "ready",
		handler: (info: { screen?: { visibleHeight?: number } }) => void,
	): void;
	close(): void;
...
export default function (pi: ExtensionAPI) {
	...
	async function loadSummaryModelChoices(
		...
		const preferredDefaults = ["ollama/gemini-3-flash-preview"];
	...
	function resolveSummaryForSubmit(
		...
		if (submittedSummary.length > 0) {
			return {
				...
				summaryMeta: normalizeSummaryMeta(
					payload.summaryMeta,
					submittedSummary,
				),
			};
		...
		const selected = filterByQueryIndices(
			payload.selectedQueryIndices,
			resultsByIndex,
		).results;
		const fallbackResults =
			selected.length > 0 ? selected : [...resultsByIndex.values()];
	...
	function buildSearchReturn(opts: SearchReturnOptions) {
		...
		const hasApprovedSummary =
			typeof opts.approvedSummary === "string" &&
	...
	function collectAllResultsAndUrls(
		resultsByIndex: Map<number, QueryResultData>,
	) {
	...
	pi.registerShortcut(curateKey, {
		...
		handler: async (ctx) => {
			...
			if (pendingCurate.phase === "searching") {
				...
				ctx.ui.notify(
					"Opening curator — remaining searches will stream in",
					"info",
				);
				return;
			}
	...
	pi.registerTool({
		name: "web_search",
		label: "Web Search",
		description: `Search the web using Perplexity AI, Exa, or Gemini. Returns an AI-synthesized answer with source citations. For comprehensive research, prefer queries (plural) with 2-4 varied angles over a single query — each query gets its own synthesized answer, so varying phrasing and scope gives much broader coverage. When includeContent is true, full page content is fetched in the background. By default, results are returned directly. Set workflow to "summary-review" to open the interactive browser curator with streaming results and an optional AI summary draft. Provider auto-selects: Exa (direct API with key, MCP fallback without), else Perplexity (needs key), else Gemini API (needs key), else Gemini Web (needs a supported Chromium-based browser login).`,
		promptSnippet:
		...
		parameters: Type.Object({
			...
			provider: Type.Optional(
				StringEnum(["auto", "perplexity", "gemini", "exa", "searxng"], {
			...
			workflow: Type.Optional(
				StringEnum(["none", "summary-review"], {
					description:
						"Search workflow mode: none = return results directly (default), summary-review = open curator with AI summary draft",
				}),
			),
		}),
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			if (shouldCurate) {
				...
				const summaryModelChoices =
					await loadSummaryModelChoices(summaryContext);
			...
			return buildSearchReturn({
				...
				inlineContent:
					allInlineContent.length > 0 ? allInlineContent : undefined,
			});
		},
		...
		renderResult(result, { expanded, isPartial }, theme) {
			...
			if (isPartial) {
				if (details?.phase === "curating") {
					return new Text(
						theme.fg("accent", "waiting for summary approval..."),
						0,
						0,
					);
				}
			...
			if (details?.fetchId && details?.fetchUrls) {
				statusLine += theme.fg(
					"muted",
					` (fetching ${details.fetchUrls.length} URLs)`,
				);
			...
			if (details?.fetchUrls && details.fetchUrls.length > 0) {
				if (details.curated) {
					lines.push(
						theme.fg(
							"muted",
							`Fetching ${details.fetchUrls.length} URLs in background`,
						),
					);
				} else {
			...
			if (!expanded) {
				...
				if (summaryPreview) {
					...
					for (const cq of details.curatedQueries.slice(0, 3)) {
						...
						box.addChild(
							new Text(theme.fg("accent", `  "${dq}"`) + suffix, 0, 0),
						);
						collapsedLines++;
					}
					...
					const fallbackLine = (firstContentLine?.trim() || "").replace(
						/\*\*/g,
						"",
					);
					if (fallbackLine) {
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate) {
			...
			onUpdate?.({
				content: [
					{ type: "text", text: `Fetching ${urlList.length} URL(s)...` },
				],
			...
			const totalChars = fetchResults.reduce(
		...
		renderCall(args, theme) {
			...
			if (timestamp) {
				lines.push(
					theme.fg("dim", "  timestamp: ") + theme.fg("warning", timestamp),
				);
			}
			if (typeof frames === "number") {
				lines.push(
					theme.fg("dim", "  frames: ") + theme.fg("warning", String(frames)),
				);
			}
			if (prompt) {
				const display =
	...
	pi.registerCommand("websearch", {
		...
		handler: async (args, ctx) => {
			...
			function sendFollowUpFromReturn(
				payload: ReturnType<typeof buildSearchReturn>,
			) {
				pi.sendMessage(
					{
	...
	pi.registerCommand("curator", {
		...
		handler: async (args, ctx) => {
			...
			if (arg.length === 0) {
				const current = resolveWorkflow(
					loadConfigForExtensionInit().workflow,
					true,
				);
				...
				ctx.ui.notify(
					`Unknown option: ${arg}. Use on, off, or summary-review.`,
					"error",
				);
				return;
			}
